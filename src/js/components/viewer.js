...
(function (g, document) {
  ...
  var runAchecker = function (isIncludeFrame, allowLogging) {
    ...
    if (isIncludeFrame) {
      for (
        i = 0, l = document.getElementsByTagName('iframe').length;
        i < l;
        i++
      ) {
        ...
        if (
          f.src === location.href ||
          (f.src.indexOf('http://') === -1 && f.src.indexOf('https://') === -1)
        ) {
        } else {
          try {
    ...
    if (cwin.document.getElementsByTagName('frameset').length > 0) {
      ...
      return {
        err: true,
        message:
          '<p>' + g.achecker.i18n.get('CannotCheckFrameset') + '</p>' + msg,
      };
    }
  ...
  var execute = function (isIncludeFrame, allowLogging) {
    if (document.getElementById('achecker-result')) {
      ...
      document.documentElement.className =
        document.documentElement.className.replace(/ achecker\-included/, '');
      return {
        err: false,
      };
    }
